...
## Sprint 3 — Shell e hubs · CONCLUÍDO em 07/09/2026 (Sessão 6)
...
**Evidências:** os 5 hubs que eram placeholder (Treinamento, Busca, Biblioteca Visual, Progresso, Referências) ganharam implementação real. Referências extrai e agrupa as 54 fontes já citadas nos 24 capítulos (nenhuma fonte nova inventada). Busca usa `aliases.json` para resultado direto determinístico — critério de aceite testado literalmente ("pull in" abre m17). Biblioteca Visual tem filtro funcional e estado vazio claro (dados reais chegam no Sprint 7, D-007/B-009). Progresso lista os 24 capítulos com estado real de conclusão e ganhou exportar/importar em JSON (B-011), com round-trip verificado em teste automatizado (exportar, limpar localStorage, importar, conferir que o estado retorna). `test_e2e.py` cresceu de 35 para 48 checagens (todas passando); `qa.py` ganhou uma correção real (falso positivo de link quebrado ao ler `href="..."` dentro de `<script>` que monta HTML via string — corrigido removendo blocos `<script>` antes de checar links).
**Fora do escopo desta sessão:** B-026 (mapeamento capítulo→fase) não foi resolvido aqui — é uma decisão de currículo (avaliar peso pedagógico de cada capítulo, decidir fusões/divisões) que cabe melhor junto da reescrita de conteúdo dos Sprints 4 a 6, não como plumbing de hub. Continua P1 no backlog.

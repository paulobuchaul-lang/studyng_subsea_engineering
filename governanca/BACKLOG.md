...
| B-036 | P1 | técnico | Ativar branch protection na `main` em Settings → Branches: `Require status checks to pass before merging` (check `qa-governanca`) E `Require a pull request before merging`, ambos marcados (Paulo confirmou as duas). Passo a passo em GUIA_PUBLICACAO.md seção 5a. Sem isso, o CI roda mas não bloqueia merge (D-020) | Pedido do Paulo, Sessão 2 | ação manual do Paulo |

...
**Atualização (mesma sessão):** o Paulo confirmou marcar também **Require a pull request before merging**, não só o status check — prioriza rigor sobre a conveniência do upload direto na `main`. GUIA_PUBLICACAO.md seções 4, 5 e 5a reescritas para o fluxo de branch temporária + PR pela própria tela de upload do GitHub (sem linha de comando). Consequência aceita conscientemente: upload direto na `main` deixa de funcionar; todo update passa por 2-3 cliques a mais (Propose changes → Create pull request → aguardar check → Merge).

...
## D-023 · 07/09/2026 · Vigente
**Decisão:** O protótipo do Sprint 2 (DESIGN_SYSTEM_V4.md) foi aplicado a todo o site (24 capítulos + 9 hubs), não apenas à amostra de 3 páginas (Home + capítulo 17 + Glossário) que o ROADMAP_V4.md original previa para aprovação antes do rollout.
**Motivo:** Diferente da V3 (HTML duplicado por página), a arquitetura de templates do Sprint 1 (D-005) faz o CSS e os templates Jinja2 serem compartilhados por todas as páginas. Redesenhar 3 páginas ou redesenhar as 33 custa exatamente o mesmo trabalho técnico — a preocupação original do ROADMAP ("evita refazer 33 páginas duas vezes") não existe mais nessa arquitetura. Aplicar a tudo dá ao Paulo mais material para avaliar sem gerar retrabalho.
**Consequência:** O critério de aceite do Sprint 2 (D-012, aprovação antes do rollout aos capítulos) muda de escopo: não é mais "aprovar 3 páginas antes de aplicar às demais", é "aprovar o design já aplicado a tudo, com ajustes entrando no backlog antes do Sprint 3" — o próprio texto do critério de aceite do Sprint 2 no ROADMAP_V4.md previa exatamente esse fluxo de ajustes pós-aprovação, então nada nele precisou mudar além do escopo da aplicação inicial.

## D-024 · 07/09/2026 · Vigente
**Decisão:** O protótipo do medidor de profundidade (Home e trilho "Neste capítulo") divide a barra em 6 zonas visuais proporcionais ao progresso geral (capítulos concluídos ÷ total), sem mapear capítulos específicos a fases nomeadas. A "Trilha" na Home continua listando os 24 capítulos na ordem da ementa (não agrupados por fase), com um aviso textual de que o agrupamento em seis fases (CLAUDE.md seção 18) ainda depende de B-026.
**Motivo:** B-026 (reavaliar a ementa e mapear os capítulos às seis fases de gamificação — Exploradora Subsea, Integradora de Sistemas etc.) é uma decisão de conteúdo que exige avaliar o peso pedagógico real de cada capítulo, ainda não feita. Fabricar uma divisão arbitrária de capítulos por fase (ex.: "capítulos 1-4 = fase 1") na interface passaria a falsa impressão de que essa decisão já foi tomada.
**Consequência:** O medidor de profundidade funciona e comunica progresso hoje (Sprint 2), mas sem nomear fases. Quando B-026 for resolvido, a Trilha passa a agrupar os capítulos pelas fases reais e o medidor ganha os marcos nomeados ("faltam N capítulos para [fase]", como o DESIGN_SYSTEM_V4.md seção 5 já prevê).

## D-025 · 07/09/2026 · Vigente
**Decisão:** O critério de D-019 para uso de inglês fica mais rígido. D-019 dizia "mantém-se o termo em inglês quando é o padrão da indústria e a tradução geraria estranheza" — na prática esse critério foi aplicado de forma frouxa demais durante a extração do Sprint 1, porque o texto de origem (V3, que por sua vez herdou de literatura técnica em inglês) já vinha em inglês e a extração copiou a forma, não só o conteúdo. O padrão passa a ser: **português é o default; inglês só sobrevive quando há evidência de que aquele termo específico é dito em inglês mesmo dentro de uma frase em português, numa reunião real de projeto no Brasil.** Não basta ser "padrão da indústria" em documentação internacional — precisa ser o que a Rafaela ouviria falado em voz alta.
**Motivo:** Evidência trazida pelo Paulo: a tabela "Sequência didática de second-end pull-in" do capítulo 17 (m17) tem etapas como "Prepare host", "Deploy messenger/main wire", "Transfer load", "Approach bellmouth/receptacle", "Seat/lock/hang-off", "Release installation rigging", "Complete connection", e uma lista com "wire/rope path, fleet angle", "control philosophy, hydraulic power, fail-safe states", "maintenance, proof load, inspection". Nenhuma pessoa fala "transfer load" ou "hydraulic power" numa reunião em português na Petrobras — diz "transferência de carga" e "força/potência hidráulica". "Pull-in" é o contra-exemplo correto: esse sim é dito em inglês mesmo em frase em português, e deve continuar em inglês. O erro não é usar inglês — é usar inglês por inércia da fonte, sem checar se é isso que se ouve de verdade.
**Critério prático de decisão, termo a termo:** perguntar "a Rafaela ouviria isso em inglês numa reunião real, ou só está em inglês porque o texto-fonte em inglês não foi traduzido?". Exemplos de termos que passam no teste (mantêm inglês, com glossário obrigatório por D-019): pull-in, pull-out, riser, jumper, umbilical, wellhead, ROV, workover, hang-off, bend stiffener, as-built, FPSO, moonpool, HPU, winch (uso comum mesmo em português). Exemplos que NÃO passam (traduzir por default): "transfer load" → transferência de carga; "hydraulic power" → força/potência hidráulica; "wire rope" → cabo de aço; "control philosophy" → filosofia de controle; "fail-safe states" → estados de segurança intrínseca/à prova de falha; "proof load" → carga de prova; "maintenance"/"inspection" → manutenção/inspeção (esses dois nem sequer são jargão, são vocabulário comum que não deveria ter sido deixado em inglês em nenhuma hipótese); nomes de etapa de tabela/processo ("Prepare host", "Complete connection" etc.) — título de etapa é sempre em português, mesmo quando algum termo dentro da etapa é um sobrevivente legítimo.
**Consequência:** Isso não é uma tarefa nova, é um aperto do critério que já rege B-030/B-031 (reescrita de conteúdo dos Sprints 4 a 6): a revisão de cada capítulo migrado passa a incluir, explicitamente, a pergunta de D-025 termo a termo, não só a checagem de cobertura de glossário de D-019. B-040 aberto no backlog citando o m17 como evidência concreta e como primeiro capítulo a corrigir. AUDITORIA_CONTEUDO_DIDATICA_V4.md seção 3.9 e a rubrica da seção 4 atualizadas com este critério.

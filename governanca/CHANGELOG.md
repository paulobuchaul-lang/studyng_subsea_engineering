...
- **Referências:** nova função em `src/build.py` extrai a seção "Aprofundamento" dos 24 `.md` (formato já consistente: "- [categoria] [Título](URL)"), agrupa por categoria e deduplica por URL citando todos os capítulos que citam a mesma fonte — 54 referências reais reunidas, nenhuma inventada.
